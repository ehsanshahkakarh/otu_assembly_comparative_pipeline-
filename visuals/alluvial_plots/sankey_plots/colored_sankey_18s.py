#!/usr/bin/env python3
"""
18S Eukaryotic Sankey Diagram with COLORED FLOWS (Python version)
=================================================================
...
print("=== 18S Eukaryotic Sankey Diagram with Colored Flows (Python) ===\n")
...
color_key = 'eukaryota_colors'

print(f"Top N divisions to show: {top_n}\n")

# Load the 18S data
merged_data_path = Path("../../../Eukcensus_merge/18s_merged/csv_results/18s_ncbi_merged_clean_phylum.csv")
if not merged_data_path.exists():
    raise FileNotFoundError(f"Cannot find 18S merged data at: {merged_data_path}")

print("Loading 18S data...")
data_18s = pd.read_csv(merged_data_path)
print(f"18S data loaded: {len(data_18s)} rows\n")

# Filter for matched divisions for Eukaryota
matched_data = data_18s[
    (data_18s['match_status'] == 'matched') &
    (data_18s['census_size_count'].notna()) &
    (data_18s['census_otu_count'].notna()) &
    (data_18s['ncbi_genome_count'].notna()) &
    (data_18s['ncbi_species_count'].notna()) &
    (data_18s['phylum'].notna()) &
    (data_18s['phylum'] != 'N/A') &
    (data_18s['phylum'] != '') &
    (data_18s['domain'] == 'Eukaryota')
].copy()

print(f"Matched eukaryotic divisions found: {len(matched_data)}\n")
...
top_divisions = matched_data.nlargest(top_n, 'total_representation')
print(f"Top {top_n} divisions selected: {len(top_divisions)}\n")
...
other_genome_count = total_genome_count - top_divisions['ncbi_genome_count'].sum()
other_size_count = total_size_count - top_divisions['census_size_count'].sum()
other_otu_count = total_otu_count - top_divisions['census_otu_count'].sum()
other_species_count = total_species_count - top_divisions['ncbi_species_count'].sum()

print("Other counts (remaining divisions):")
...
# Get division names and assign colors
division_names = list(top_divisions['phylum']) + ['Other']
...
for division in division_names:
    if division == 'Other':
        division_colors.append(other_color)
    elif division in domain_colors:
        division_colors.append(domain_colors[division])
    else:
        # Fallback color
        fallback_colors = color_config['fallback_colors']['eukaryota']
        idx = len(division_colors) % len(fallback_colors)
        division_colors.append(fallback_colors[idx])

print("Assigning colors from shared taxonomic color mapping...")
print("\nCOLOR MAPPING SUMMARY:")
print("=" * 60)
for i, (division, color) in enumerate(zip(division_names, division_colors), 1):
    print(f"{i:2d}. {division:30s} {color}")
print("=" * 60)
print()
...
for stage_idx, stage in enumerate(stages):
    for division_idx, division in enumerate(division_names):
        node_labels.append(f"{division}")
        node_colors.append(division_colors[division_idx])
        node_x.append(x_positions[stage_idx])
        node_y.append(division_idx / len(division_names))
        node_map[(division, stage)] = node_id
        node_id += 1
...
def add_link(division, source_stage, target_stage, value, color):
    if value > 0:
        sources.append(node_map[(division, source_stage)])
        targets.append(node_map[(division, target_stage)])
        values.append(value)
...
# Create links for top divisions
for idx, row in top_divisions.iterrows():
    division = row['phylum']
    color = division_colors[division_names.index(division)]

    # Genomes -> Sequences
    add_link(division, "Genomes", "Sequences", row['census_size_count'], color)

    # Sequences -> OTUs
    add_link(division, "Sequences", "OTUs", row['census_otu_count'], color)

    # OTUs -> Species
    add_link(division, "OTUs", "Species", row['ncbi_species_count'], color)
...
fig.update_layout(
    title=dict(
        text=f"Eukaryota Sankey Diagram with Colored Flows<br>" +
             f"<sub>Genomes → Sequences → OTUs → Species | Top {top_n} Divisions + Other</sub>",
        font=dict(size=20)
    ),
...
print("=" * 70)
print("🧬 EUKARYOTA SANKEY DIAGRAM WITH COLORED FLOWS - SUMMARY")
print("=" * 70)
print()
print("📊 DATASET OVERVIEW:")
print("   • Domain: Eukaryota")
print(f"   • Total divisions analyzed: {len(top_divisions)} + Other")
...
print("   • COLORED FLOWS matching division colors")
print(f"   • Color-coded divisions with {len(division_colors)} distinct colors")
print("   • Flow widths represent absolute counts")
print("   • HTML output for web viewing and sharing")
print()
print("📋 FLOW SUMMARY:")
print("-" * 80)
print(f"{'Division':<30s} {'Genomes':>12s} {'Sequences':>12s} {'OTUs':>12s} {'Species':>12s} {'Color':>10s}")
print("-" * 80)

for idx, row in top_divisions.iterrows():
    division = row['phylum']
    color = division_colors[division_names.index(division)]
    print(f"{division[:30]:<30s} {row['ncbi_genome_count']:>12,} {row['census_size_count']:>12,} "
...
print("-" * 80)
print()
print("✅ EUKARYOTA Sankey diagram with colored flows completed!")
print(f"   📁 HTML file: {output_file}")
print("   💡 Open in web browser to view interactive diagram")
print("   🎨 Flows are colored to match their division colors")
print("=" * 70)
