#!/usr/bin/env Rscript
# 16S Bacteria Phylum Focused Scatter Plot
# Created: 2025-08-18
# Purpose: Clean, focused scatter plot for 16S Bacteria at Phylum level only
...
script_dir <- dirname(normalizePath(ifelse(interactive(), 
                                          file.path(getwd(), "scatter_16s_bacteria_phylum_focused.R"),
...
# Clean configuration for 16S Bacteria Phylum analysis
config <- list(
...
# Load and process 16S bacteria phylum data
load_bacteria_phylum_data <- function() {
  ...
  if ("domain" %in% colnames(data)) {
    ...
    cat(paste("Found", nrow(data), "bacteria phyla after filtering by domain column\n"))
  } else {
    ...
    cat(paste("Found", nrow(data), "bacteria phyla after filtering archaea by pattern matching\n"))
  }

  # Filter for phyla with meaningful data (either Census OR NCBI data, but both > 0 for ratios)
  # Keep all phyla but we'll filter ratios later
  data_with_both <- data %>% filter(Census_OTU_Count > 0, NCBI_Species_Count > 0)
  cat(paste("Found", nrow(data_with_both), "bacteria phyla with both Census and NCBI data\n"))
  cat(paste("Total bacteria phyla in dataset:", nrow(data), "\n"))

  # Use the subset with both for ratio calculations
  data <- data_with_both

  # For phylum level, the phylum IS the taxon name
  ...
  # Debug: Check Bacillota specifically
  bacillota_row <- data[data$Taxon == "Bacillota", ]
  if (nrow(bacillota_row) > 0) {
    cat(sprintf("Bacillota novelty ratio: %.2f (Census: %d, NCBI: %d)\n",
                bacillota_row$Novelty_Ratio, bacillota_row$Census_OTU_Count, bacillota_row$NCBI_Species_Count))
    cat(sprintf("Bacillota coverage ratio: %.2f\n", bacillota_row$Coverage_Factor))
  }
  ...
  # Mark top novelty (only from those with ratio > 1.0) - FIXED RANKING BUG
  ...
  # Mark top coverage (only from those with ratio > 1.0) - FIXED RANKING BUG
...
create_bacteria_phylum_scatter <- function(data) {
  ...
  p <- ggplot(data, aes(x = Census_OTU_Count, y = NCBI_Species_Count)) +
    scale_x_log10(labels = comma_format(),
                  limits = c(1, 100000),  # Wider range for phylum-level data
  ...
  # Filter out taxa with both novelty and coverage factors < 1.0 (not meaningful)
  if (config$use_weighted_ratio) {
  ...
  # Specifically exclude phyla that are not interesting despite having factors > 1.0
  # Bacillota: novelty 3.46, coverage 0.29 - not as interesting as extreme cases
  ...
  cat(paste("Top taxa with factors > 1.0 (after exclusions):", nrow(meaningful_data), "out of", nrow(top_data), "\n"))
  cat(paste("Excluded phyla:", paste(excluded_phyla, collapse = ", "), "\n"))
  ...
  p <- p +
    ...
    labs(title = "16S EukCensus vs NCBI: Bacteria Phylum",
         x = "16S EukCensus OTU Count", y = "NCBI Species Count",
         caption = paste(nrow(data), "bacteria phyla"))
...
# Main function
main <- function() {
  cat("16S Bacteria Phylum Focused Analysis\n")
  cat("===================================\n")

  # Load and process data
  data <- load_bacteria_phylum_data()

  # Create plot
  plot <- create_bacteria_phylum_scatter(data)

  # Save plot and summaries
  ggsave(file.path(config$output_dir, "16s_bacteria_phylum_focused.png"), plot,
         width = config$plot_width, height = config$plot_height, dpi = config$dpi, bg = "white")

  write.csv(generate_summary(data, "coverage"),
            file.path(config$output_dir, "16s_bacteria_phylum_top_coverage.csv"), row.names = FALSE)
  write.csv(generate_summary(data, "novelty"),
            file.path(config$output_dir, "16s_bacteria_phylum_top_novelty.csv"), row.names = FALSE)

  cat(paste("✅ Analysis complete! Processed", nrow(data), "bacteria phyla\n"))
  cat(paste("   Top novelty taxa:", sum(data$Is_Top_Novelty), "\n"))
  cat(paste("   Top coverage taxa:", sum(data$Is_Top_Coverage), "\n"))
  cat("\nGenerated files:\n")
  cat("  - 16s_bacteria_phylum_focused.png\n")
  cat("  - 16s_bacteria_phylum_top_coverage.csv\n")
  cat("  - 16s_bacteria_phylum_top_novelty.csv\n")
}
