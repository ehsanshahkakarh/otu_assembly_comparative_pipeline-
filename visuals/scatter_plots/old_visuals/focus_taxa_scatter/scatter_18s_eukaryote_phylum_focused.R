#!/usr/bin/env Rscript
# 18S Eukaryote Division Focused Scatter Plot
# Created: 2025-08-18
# Purpose: Clean, focused scatter plot for 18S Eukaryotes at Division level only
...
script_dir <- dirname(normalizePath(ifelse(interactive(),
                                          file.path(getwd(), "scatter_18s_eukaryote_phylum_focused.R"),
...
# Clean configuration for 18S Eukaryote Phylum analysis
config <- list(
...
get_division_mapping <- function() {
  ...
  # Also load phylum-level census data to create phylum -> division mapping
  phylum_file <- file.path("..", "..", "18S_censusparse", "csv_outputs", "eukcensus_18S_by_phylum.csv")
  if (!file.exists(phylum_file)) {
    cat(paste("Warning: Census phylum file not found:", phylum_file, "\n"))
    return(NULL)
  }

  phylum_data <- read.csv(phylum_file, stringsAsFactors = FALSE)
  cat(paste("Loaded census phylum file with", nrow(phylum_data), "phyla\n"))

  # Create phylum -> division mapping by parsing lineages
  phylum_to_division <- setNames(character(nrow(phylum_data)), phylum_data$Name_to_use)
  successful_mappings <- 0

  # Create a lookup of division names for matching
  division_names <- division_data$Name_to_use

  for (i in 1:nrow(phylum_data)) {
    phylum_name <- phylum_data$Name_to_use[i]
    lineage <- phylum_data$lineage[i]
    ...
    if (!is.na(lineage) && lineage != "") {
      ...
      phylum_to_division[phylum_name] <- division_match
      if (division_match != "Other") {
      ...
      if (successful_mappings <= 5) {
        cat(paste("Example mapping:", phylum_name, "->", division_match, "\n"))
      }
    } else {
      phylum_to_division[phylum_name] <- "Other"
    }
  }

  cat(paste("Successfully mapped", successful_mappings, "phyla to divisions\n"))
  unique_divisions <- unique(phylum_to_division[phylum_to_division != "Other"])
  cat(paste("Unique divisions found:", length(unique_divisions), "\n"))
  cat(paste("Divisions:", paste(unique_divisions, collapse = ", "), "\n"))

  return(phylum_to_division)
}
...
# Load and process 18S eukaryote division data
load_eukaryote_division_data <- function() {
  filepath <- file.path(config$data_dir, "18s_ncbi_merged_clean_phylum.csv")
  ...
  cat(paste("Found", nrow(data), "eukaryotic divisions\n"))

  # Filter for divisions with meaningful data (both Census AND NCBI data > 0 for ratios)
  data_with_both <- data %>% filter(Census_OTU_Count > 0, NCBI_Species_Count > 0)
  cat(paste("Found", nrow(data_with_both), "divisions with both Census and NCBI data\n"))
  cat(paste("Total divisions in dataset:", nrow(data), "\n"))

  # Use the subset with both for ratio calculations
  data <- data_with_both

  # Since these are already divisions, use the Taxon name as the Division
  ...
  cat(paste("Divisions with novelty ratio > 1.0:", nrow(high_novelty_data), "\n"))
  cat(paste("Divisions with coverage ratio > 1.0:", nrow(high_coverage_data), "\n"))
  ...
  if (nrow(high_novelty_data) > 0) {
    ...
    cat(paste("Selected top", length(top_novelty_taxa), "novelty divisions:", paste(top_novelty_taxa, collapse = ", "), "\n"))

    # Debug: Show all novelty ratios > 1.0 sorted
    cat("All divisions with novelty > 1.0 (sorted by novelty ratio):\n")
  ...
  if (nrow(high_coverage_data) > 0) {
    ...
    cat(paste("Selected top", length(top_coverage_taxa), "coverage divisions:", paste(top_coverage_taxa, collapse = ", "), "\n"))
  }

  # Debug: Print top coverage and novelty taxa
  cat("\n=== TOP COVERAGE DIVISIONS ===\n")
...
create_eukaryote_division_scatter <- function(data) {
  # Base plot with professional styling - wider limits for division level
  p <- ggplot(data, aes(x = Census_OTU_Count, y = NCBI_Species_Count)) +
    scale_x_log10(labels = comma_format(),
                  limits = c(1, 100000),  # Range for division-level data (wider)
                  expand = expansion(mult = c(0.05, 0.05))) +
    scale_y_log10(labels = comma_format(),
                  limits = c(1, 10000),  # Range for division-level data (wider)
                  expand = expansion(mult = c(0.05, 0.05)))

  # Remove background points - only show highlighted taxa with factors > 1.0
  # Highlighted points with colors by divisions
  top_data <- data[data$Is_Top_Novelty | data$Is_Top_Coverage, ]

  # Show all top divisions without additional filtering
  meaningful_data <- top_data

  cat(paste("Showing all top divisions:", nrow(meaningful_data), "out of", nrow(top_data), "\n"))
  top_data <- meaningful_data

  if (nrow(top_data) > 0) {
    # Since we're working with divisions directly, each taxon IS a division
    # Show only the meaningful divisions in the legend
    top_divisions <- unique(top_data$Taxon)  # Use Taxon since each row is a division
    top_divisions <- sort(top_divisions)
    n_divisions <- length(top_divisions)

    cat(paste("Showing", n_divisions, "meaningful divisions with individual colors:", paste(top_divisions, collapse = ", "), "\n"))
    ...
    # Add colored fill layer on top - color by taxon (which is the division name)
    p <- p + geom_point(data = top_data, aes(size = Circle_Size * 0.8, color = factor(Taxon, levels = top_divisions)),
                       alpha = 0.9) +
  ...
  p <- p +
    ...
    labs(title = "18S EukCensus vs NCBI: Eukaryote Division",
         x = "18S EukCensus OTU Count", y = "NCBI Species Count",
         caption = paste(nrow(data), "eukaryotic divisions"))

  # Add circle size legend - positioned up and to the left
  x_range <- range(data$Census_OTU_Count, na.rm = TRUE)
  y_range <- range(data$NCBI_Species_Count, na.rm = TRUE)

  legend_x <- 10^(log10(x_range[1]) + 0.02 * diff(log10(x_range)))  # Move slightly to the right
  legend_y_top <- 10^(log10(y_range[2]) - 0.08 * diff(log10(y_range)))  # Move down a bit more from top

  # Add circle size legend title
  ...
  for (i in 1:3) {
    ...
    # Place labels to the left of circles
...
# Main function
main <- function() {
  cat("18S Eukaryote Division Focused Analysis\n")
  cat("======================================\n")

  # Load and process data
  data <- load_eukaryote_division_data()

  # Create plot
  plot <- create_eukaryote_division_scatter(data)

  # Save plot and summaries
  ggsave(file.path(config$output_dir, "18s_eukaryote_division_focused.png"), plot,
         width = config$plot_width, height = config$plot_height, dpi = config$dpi, bg = "white")

  write.csv(generate_summary(data, "coverage"),
            file.path(config$output_dir, "18s_eukaryote_division_top_coverage.csv"), row.names = FALSE)
  write.csv(generate_summary(data, "novelty"),
            file.path(config$output_dir, "18s_eukaryote_division_top_novelty.csv"), row.names = FALSE)

  cat(paste("✅ Analysis complete! Processed", nrow(data), "eukaryotic divisions\n"))
  cat(paste("   Top novelty taxa:", sum(data$Is_Top_Novelty), "\n"))
  cat(paste("   Top coverage taxa:", sum(data$Is_Top_Coverage), "\n"))
  cat("\nGenerated files:\n")
  cat("  - 18s_eukaryote_division_focused.png\n")
  cat("  - 18s_eukaryote_division_top_coverage.csv\n")
  cat("  - 18s_eukaryote_division_top_novelty.csv\n")
}
