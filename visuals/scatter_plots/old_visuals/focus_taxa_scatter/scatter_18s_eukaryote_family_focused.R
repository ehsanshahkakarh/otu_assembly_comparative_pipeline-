#!/usr/bin/env Rscript
# 18S Eukaryote Family Focused Scatter Plot
# Created: 2025-08-18
# Purpose: Clean, focused scatter plot for 18S Eukaryotes at Family level only
...
script_dir <- dirname(normalizePath(ifelse(interactive(), 
                                          file.path(getwd(), "scatter_18s_eukaryote_family_focused.R"),
...
# Clean configuration for 18S Eukaryote Family analysis
config <- list(
  data_dir = file.path("..", "Eukcensus_merge", "18s_merged", "csv_results"),
  ...
  use_weighted_ratio = FALSE  # TOGGLE: Set to FALSE to use simple ratios, TRUE for weighted ratios
)
...
get_division_mapping <- function() {
  ...
  # Also load family-level census data to create family -> division mapping
  family_file <- file.path("..", "..", "18S_censusparse", "csv_outputs", "eukcensus_18S_by_family.csv")
  if (!file.exists(family_file)) {
    cat(paste("Warning: Census family file not found:", family_file, "\n"))
    return(NULL)
  }

  family_data <- read.csv(family_file, stringsAsFactors = FALSE)
  cat(paste("Loaded census family file with", nrow(family_data), "families\n"))

  # Create family -> division mapping by parsing lineages
  family_to_division <- setNames(character(nrow(family_data)), family_data$Name_to_use)
  ...
  for (i in 1:nrow(family_data)) {
    family_name <- family_data$Name_to_use[i]
    lineage <- family_data$lineage[i]
    ...
    if (!is.na(lineage) && lineage != "") {
      ...
      family_to_division[family_name] <- division_match
      if (division_match != "Other") {
      ...
      if (successful_mappings <= 5) {
        cat(paste("Example mapping:", family_name, "->", division_match, "\n"))
      }
    } else {
      family_to_division[family_name] <- "Other"
    }
  }

  cat(paste("Successfully mapped", successful_mappings, "families to divisions\n"))
  unique_divisions <- unique(family_to_division[family_to_division != "Other"])
  cat(paste("Unique divisions found:", length(unique_divisions), "\n"))
  cat(paste("Divisions:", paste(unique_divisions, collapse = ", "), "\n"))

  return(family_to_division)
}
...
# Load and process 18S eukaryote family data
load_eukaryote_family_data <- function() {
  filepath <- file.path(config$data_dir, "18s_ncbi_merged_clean_family.csv")
  ...
  cat(paste("Found", nrow(data), "eukaryotic families\n"))

  # Filter for families with meaningful data (both Census AND NCBI data > 0 for ratios)
  data_with_both <- data %>% filter(Census_OTU_Count > 0, NCBI_Species_Count > 0)
  cat(paste("Found", nrow(data_with_both), "families with both Census and NCBI data\n"))
  ...
  cat(paste("Families with novelty ratio > 1.0:", nrow(high_novelty_data), "\n"))
  cat(paste("Families with coverage ratio > 1.0:", nrow(high_coverage_data), "\n"))
  ...
  if (nrow(high_novelty_data) > 0) {
    ...
    cat("All families with novelty > 1.0 (sorted by novelty ratio):\n")
...
create_eukaryote_family_scatter <- function(data) {
  ...
  p <- ggplot(data, aes(x = Census_OTU_Count, y = NCBI_Species_Count)) +
    scale_x_log10(labels = comma_format(),
                  limits = c(1, 10000),  # Range for family-level data
  ...
  # Apply stricter cutoff: families must be significantly away from 1:1 line (1x ± 0.5)
  # Novelty ratio > 1.5 OR Coverage ratio > 1.5 (avoid families too close to diagonal)
  if (config$use_weighted_ratio) {
  ...
  # Specifically exclude very common/uninteresting families (user can customize this list)
  # These might have factors > 1.5 but are not as interesting as extreme cases
  excluded_families <- c("Insecta", "Mammalia", "Teleostei")  # Very common, large groups
  meaningful_data <- meaningful_data[!meaningful_data$Taxon %in% excluded_families, ]

  cat(paste("Top families with factors > 1.5 (after exclusions):", nrow(meaningful_data), "out of", nrow(top_data), "\n"))
  cat(paste("Excluded families:", paste(excluded_families, collapse = ", "), "\n"))
  ...
  if (nrow(top_data) > 0) {
    ...
    # Add colored fill layer on top - color by division, not individual family
  ...
  p <- p +
    ...
    labs(title = "18S EukCensus vs NCBI: Eukaryote Family",
         x = "18S EukCensus OTU Count", y = "NCBI Species Count",
         caption = paste(nrow(data), "eukaryotic families"))

  # Add circle size legend at the top left corner
  x_range <- range(data$Census_OTU_Count, na.rm = TRUE)
  y_range <- range(data$NCBI_Species_Count, na.rm = TRUE)

  legend_x <- 10^(log10(x_range[1]) - 0.02 * diff(log10(x_range)))  # Even further left (outside plot area)
  legend_y_top <- 10^(log10(y_range[2]) + 0.05 * diff(log10(y_range)))  # Even further up (outside plot area)
...
# Main function
main <- function() {
  cat("18S Eukaryote Family Focused Analysis\n")
  cat("====================================\n")

  # Load and process data
  data <- load_eukaryote_family_data()

  # Create plot
  plot <- create_eukaryote_family_scatter(data)

  # Save plot and summaries
  ggsave(file.path(config$output_dir, "18s_eukaryote_family_focused.png"), plot,
         width = config$plot_width, height = config$plot_height, dpi = config$dpi, bg = "white")

  write.csv(generate_summary(data, "coverage"),
            file.path(config$output_dir, "18s_eukaryote_family_top_coverage.csv"), row.names = FALSE)
  write.csv(generate_summary(data, "novelty"),
            file.path(config$output_dir, "18s_eukaryote_family_top_novelty.csv"), row.names = FALSE)

  cat(paste("✅ Analysis complete! Processed", nrow(data), "eukaryotic families\n"))
  cat(paste("   Top novelty taxa:", sum(data$Is_Top_Novelty), "\n"))
  cat(paste("   Top coverage taxa:", sum(data$Is_Top_Coverage), "\n"))
  cat("\nGenerated files:\n")
  cat("  - 18s_eukaryote_family_focused.png\n")
  cat("  - 18s_eukaryote_family_top_coverage.csv\n")
  cat("  - 18s_eukaryote_family_top_novelty.csv\n")
}
