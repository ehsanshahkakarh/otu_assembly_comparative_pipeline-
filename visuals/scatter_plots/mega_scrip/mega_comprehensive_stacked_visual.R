...
# ============================================================================
# SIMPLE COLOR ASSIGNMENT - Just uses YAML colors, that's it!
# ============================================================================
...
assign_taxon_color <- function(taxon, domain, color_config) {
  ...
  # Not hardcoded? Get from cross-domain fallback pool
  if (is.null(color)) {
    # Get fallback pool (cross-domain recycling)
    if (domain == "Bacteria") {
    ...
    # Simple round-robin: use number of taxa we've already assigned
...
config <- list(
  ...
  source_data_dir = "source_data",  # Will be created in mega_scrip
...
main <- function() {
  ...
  init_color_registry()
  cat("✅ Initialized color assignment (YAML colors + cross-domain recycling)\n")
