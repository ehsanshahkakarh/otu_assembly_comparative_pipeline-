#!/usr/bin/env python3
"""
GTDB Genus Parser (Species Subset Version)

This script processes GTDB taxonomy files to extract genus-level information.
It subsets data to one genome per genus-species combination, then counts the number of species per genus
...
Output:
- Summary CSV with genus species counts (gtdb_genus_species_counts.csv)

Usage:
    python genus_gtdb_parse_species_subset.py [--input-dir INPUT_DIR] [--output-dir OUTPUT_DIR] [--log-level {DEBUG,INFO,WARNING,ERROR,CRITICAL}]
"""
...
SUMMARY_OUTPUT_FILENAME = "gtdb_genus_species_counts.csv"
...
# Removed merge_related_genera function - not used in species subset analysis
...
def get_taxids_from_names(genus_names: List[str], use_gtdb: bool = True) -> Dict[str, str]:
    """
    Get taxids for genus names using taxonkit name2taxid.
    ...
    Returns:
        Dictionary mapping genus names to their taxids
    """
    if not genus_names:
        return {}

    db_type = "GTDB" if use_gtdb else "NCBI"
    print(f"Getting {db_type} taxids for {len(genus_names)} genus names...")
    ...
    print(f"Successfully processed {len(name_to_taxid)} genus name->{db_type} taxid mappings")
...
# Removed get_taxids_from_names_ncbi_fallback and get_lineages_from_ncbi_taxids - consolidated into generic functions
...
def main() -> int:
    ...
    try:
        with timer("Total execution", metrics_list) as total_metrics:
            # Set up paths
            ...
            with timer("Processing counts", metrics_list) as metrics:
                ...
                # Step 1: First count species for each original genus (before cleaning)
                # logging.info("Counting species for each original genus...")
                initial_count = len(df)
                species_subset_df = df.drop_duplicates(subset=["genus", "species"], keep="first")
                # logging.info(f"✅ Reduced from {initial_count:,} genomes to {len(species_subset_df):,} unique genus-species combinations")

                # Count species per original genus
                original_counts = species_subset_df.groupby(["genus", "domain"])["species"].nunique().reset_index(name="genus_species_count")
                # logging.info(f"🔍 Found species counts for {len(original_counts)} original genus-domain combinations")

                # Step 2: Clean genus names and sum counts for merged genera
                # logging.info("Cleaning genus names and summing counts...")
                original_counts['genus_clean'] = original_counts['genus'].str.replace(r'_[A-Z]$', '', regex=True)

                # Sum species counts for genera that get merged after cleaning
                counts = original_counts.groupby(["genus_clean", "domain"])["genus_species_count"].sum().reset_index()
            ...
            with timer("Processing counts continued", metrics_list) as metrics:
                # Sort counts by species count
                counts = counts.sort_values("genus_species_count", ascending=False)
                ...
                # Show some examples of cleaned names and count merging
                # logging.info("📝 Examples of genus merging:")
                # merged_examples = original_counts[original_counts['genus'] != original_counts['genus_clean']].head(5)
                # for _, row in merged_examples.iterrows():
                #     logging.info(f"  {row['genus']} → {row['genus_clean']} ({row['genus_species_count']} species)")

                # Show examples of genera that got merged together
                # merged_genera = counts[counts['genus_species_count'] > original_counts['genus_species_count'].max()].head(3)
                # if not merged_genera.empty:
                #     logging.info("📝 Examples of merged genus counts:")
                #     for _, row in merged_genera.iterrows():
                #         original_parts = original_counts[original_counts['genus_clean'] == row['genus']]
                #         part_names = original_parts['genus'].tolist()
                #         part_counts = original_parts['genus_species_count'].tolist()
                #         logging.info(f"  {row['genus']}: {row['genus_species_count']} species (from {part_names} with counts {part_counts})")

                # Get taxids for genus names using GTDB taxdump
                logging.info("Getting GTDB taxids for genus names...")
                genus_to_taxid = get_taxids_from_names(unique_genera, use_gtdb=True)
                ...
                if not failed_matches.empty:
                    ...
                    if not final_failed.empty:
                        ...
                        total_species = counts['genus_species_count'].sum()
                        failed_species = final_failed['genus_species_count'].sum()
                        ...
                        with open(log_file, 'w') as f:
                            ...
                            f.write(f"Total species across all genera: {total_species:,}\n")
                            f.write(f"Species in failed genera: {failed_species:,}\n")
                            f.write(f"Percentage of species lost due to failed mapping: {lost_species_percentage:.1f}%\n\n")

                            f.write("=== FAILED GENERA DETAILS ===\n")
                            f.write("# Format: genus_name,domain,species_count,percentage_of_total_species\n")

                            for _, row in final_failed.iterrows():
                                species_pct = (row['genus_species_count'] / total_species) * 100
                                f.write(f"{row['genus']},{row['domain']},{row['genus_species_count']},{species_pct:.2f}%\n")

                        logging.info(f"📝 Enhanced failed mapping log saved to: {log_file}")
                        logging.info(f"📊 Data loss summary: {final_failed_count}/{total_genera} genera ({final_failure_rate:.1f}%), {failed_species:,}/{total_species:,} species ({lost_species_percentage:.1f}%)")
                    ...
                    if not final_failed.empty:
                        top_failed = final_failed.nlargest(5, 'genus_species_count')
                        logging.info("🔍 Top final failed matches by species count:")
                        for _, row in top_failed.iterrows():
                            logging.info(f"  {row['genus']} ({row['domain']}): {row['genus_species_count']} species")
