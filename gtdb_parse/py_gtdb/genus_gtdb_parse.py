...
This script processes GTDB taxonomy files to extract genus-level information.
It counts the number of genomes per genus and creates output files with
...
- Detailed CSV with accessions (gtdb_genus_with_accessions.csv)

Usage:
    python genus_gtdb_parse.py [--input-dir INPUT_DIR] [--output-dir OUTPUT_DIR] [--log-level {DEBUG,INFO,WARNING,ERROR,CRITICAL}]
"""
...
MIN_GENOME_THRESHOLD = 10  # Minimum number of genomes required for a genus to be included
...
def setup_logging(log_level: str, output_dir: Path) -> None:
    ...
    log_file = log_dir / f"gtdb_genus_parse_{timestamp}.log"
...
def parse_arguments() -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(description="Parse GTDB taxonomy files for genus-level information")
    ...
    parser.add_argument("--min-genomes",
                       ...
                       help=f"Minimum number of genomes required for a genus to be included (default: {MIN_GENOME_THRESHOLD})")
...
def process_taxonomy_data(df: pd.DataFrame, taxid_map: Dict[str, str] = None) -> pd.DataFrame:
    """
    Process taxonomy data to extract genus information and other ranks.
    ...
    Returns:
        pd.DataFrame: Processed DataFrame with genus information and other ranks
    """
...
def standardize_genus_name(genus: str) -> str:
    """
    Standardize genus name format.

    Args:
        genus (str): Genus name to standardize

    Returns:
        str: Standardized genus name
    """
    if pd.isna(genus) or genus == "Unclassified":
        return "Unclassified"
...
def merge_related_genera(counts_df: pd.DataFrame) -> pd.DataFrame:
    """
    Merge related genera based on naming patterns.

    Args:
        counts_df (pd.DataFrame): DataFrame with genus counts

    Returns:
        pd.DataFrame: DataFrame with merged genera
    """
    # Create a copy to avoid modifying the original
    df = counts_df.copy()

    # Define patterns for related genera
    patterns = {
        r'^Escherichia.*$': 'Escherichia',
        r'^Pseudomonas.*$': 'Pseudomonas',
        r'^Bacillus.*$': 'Bacillus',
        r'^Staphylococcus.*$': 'Staphylococcus',
        r'^Streptococcus.*$': 'Streptococcus',
        r'^Lactobacillus.*$': 'Lactobacillus',
        r'^Clostridium.*$': 'Clostridium',
        r'^Bacteroides.*$': 'Bacteroides',
        r'^Prevotella.*$': 'Prevotella',
        r'^Ruminococcus.*$': 'Ruminococcus',
        r'^Lachnospira.*$': 'Lachnospira',
        r'^Bifidobacterium.*$': 'Bifidobacterium',
        r'^Corynebacterium.*$': 'Corynebacterium',
        r'^Mycobacterium.*$': 'Mycobacterium',
        r'^Actinomyces.*$': 'Actinomyces',
        r'^Streptomyces.*$': 'Streptomyces',
        r'^Micrococcus.*$': 'Micrococcus',
        r'^Propionibacterium.*$': 'Propionibacterium'
    }

    # Apply standardization
    df['genus'] = df['genus'].apply(standardize_genus_name)

    # Apply pattern matching and merging
    for pattern, replacement in patterns.items():
        mask = df['genus'].str.match(pattern, na=False)
        df.loc[mask, 'genus'] = replacement

    # Group by the standardized genus names and sum the counts
    df = df.groupby(['genus', 'domain']).agg({
...
def validate_taxonomy(df: pd.DataFrame) -> bool:
    ...
    try:
        ...
        critical_columns = ["accession_clean", "domain", "genus"]
...
def get_taxids_from_names_gtdb(genus_names: List[str]) -> Dict[str, str]:
    """
    Get taxids for genus names using GTDB taxdump via taxonkit name2taxid.

    Args:
        genus_names: List of genus names

    Returns:
        Dictionary mapping genus names to their GTDB taxids
    """
    if not genus_names:
        return {}

    print(f"Getting GTDB taxids for {len(genus_names)} genus names...")
    ...
    # Create temporary file for genus names
    ...
    print(f"Successfully processed {len(name_to_taxid)} genus name->GTDB taxid mappings")
...
def get_taxids_from_names_ncbi_fallback(genus_names: List[str]) -> Dict[str, str]:
    """
    Fallback function to get taxids for genus names using NCBI taxonkit.
    Used for names that failed GTDB taxdump lookup.

    Args:
        genus_names: List of genus names that failed GTDB lookup

    Returns:
        Dictionary mapping genus names to their NCBI taxids
    """
    if not genus_names:
        return {}

    print(f"🔄 Fallback: Getting NCBI taxids for {len(genus_names)} failed GTDB names...")
...
def cleanup_lineage_to_genus_level(lineage: str, lineage_ranks: str, lineage_taxids: str) -> Tuple[str, str, str]:
    """
    Clean up lineage data to only include taxonomic levels up to genus level.
    Removes any ranks below genus (species).
    ...
    Returns:
        Tuple of (cleaned_lineage, cleaned_ranks, cleaned_taxids) with only genus-level and above
    """
    ...
    try:
        ...
        # If we found a genus rank, truncate everything after it
        if genus_idx >= 0:
            # Keep everything up to and including genus level
            cleaned_lineage = ';'.join(lineage_parts[:genus_idx + 1])
            cleaned_ranks = ';'.join(ranks_parts[:genus_idx + 1])
            cleaned_taxids = ';'.join(taxids_parts[:genus_idx + 1])

            return cleaned_lineage, cleaned_ranks, cleaned_taxids

        # If no genus rank found, return original (this preserves existing behavior)
    ...
    try:
        with timer("Total execution", metrics_list):
            ...
            with timer("Processing counts", metrics_list) as metrics:
                # Drop entries missing genus
                df = df.dropna(subset=["genus"])
                # logging.info(f"✅ After dropping null genera: {len(df):,} rows")

                # Enable tqdm for pandas operations
                tqdm.pandas(desc="Processing genera")

                # Clean genus names by removing underscores and suffixes (e.g., Dorea_A -> Dorea)
                ...
                # Clean genus names first
                all_genomes_df['genus_clean'] = all_genomes_df['genus'].str.replace(r'_[A-Z]$', '', regex=True)

                # Step 2: Group by cleaned genus and domain, and count all genomes
                # logging.info("Counting genomes by cleaned genus and domain...")
                counts = all_genomes_df.groupby(["genus_clean", "domain"]).size().reset_index(name="genus_counts")
                counts = counts.rename(columns={'genus_clean': 'genus'})

            # Save detailed file with all genomes (no species subsetting)
            with timer("Saving detailed file", metrics_list) as metrics:
                detailed_df = all_genomes_df.dropna(subset=["genus"])
                # Use cleaned genus names for the detailed file
                detailed_df['genus'] = detailed_df['genus_clean']
                # Sort by domain and genus for detailed file
                detailed_df = detailed_df.sort_values(["domain", "genus", "taxid"], ascending=[True, True, True])
                detailed_df[["accession_clean", "genus", "domain", "taxid"]].to_csv(detailed_output_file, index=False)
            ...
            with timer("Processing counts continued", metrics_list) as metrics:
                # Sort counts by genome count
                counts = counts.sort_values("genus_counts", ascending=False)

                # Get unique genus names for taxonkit processing
                unique_genera = counts['genus'].unique().tolist()
                # logging.info(f"🔍 Found {len(unique_genera)} unique cleaned genera")

                # Get taxids for genus names using GTDB taxdump
                # logging.info("Getting GTDB taxids for genus names...")
                genus_to_taxid = get_taxids_from_names_gtdb(unique_genera)

                # Add taxid column to counts
                counts['taxid'] = counts['genus'].map(genus_to_taxid)

                # Try NCBI fallback for failed GTDB matches
                failed_matches = counts[counts['taxid'].isna()]
                if not failed_matches.empty:
                    logging.info(f"⚠️  Failed to get GTDB taxids for {len(failed_matches)} genera")

                    # Try NCBI fallback for failed names
                    failed_names = failed_matches['genus'].unique().tolist()
                    ...
                    for idx, row in failed_matches.iterrows():
                        ...
                        if genus_name in ncbi_fallback_taxids:
                            counts.at[idx, 'taxid'] = ncbi_fallback_taxids[genus_name]
                    ...
                    if not final_failed.empty:
                        log_file = "gtdb_genus_final_failed_taxids.log"

                        # Calculate comprehensive statistics
                        total_genera = len(counts)
                        final_failed_count = len(final_failed)
                        final_success_count = total_genera - final_failed_count
                        final_success_rate = (final_success_count / total_genera) * 100
                        final_failure_rate = (final_failed_count / total_genera) * 100

                        # Calculate genome impact
                        total_genomes = counts['genus_counts'].sum()
                        failed_genomes = final_failed['genus_counts'].sum()
                        lost_genomes_percentage = (failed_genomes / total_genomes) * 100

                        with open(log_file, 'w') as f:
                            f.write("# GTDB Genus Names That Failed Both GTDB and NCBI Taxid Lookup\n")
                            f.write("# These names could not be found in either GTDB taxdump or NCBI taxonomy\n")
                            f.write("# Generated: " + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + "\n\n")

                            f.write("=== GENUS LEVEL MAPPING STATISTICS ===\n")
                            f.write(f"Total genera processed: {total_genera}\n")
                            ...
                            f.write(f"Total genomes across all genera: {total_genomes:,}\n")
                            f.write(f"Genomes in failed genera: {failed_genomes:,}\n")
                            f.write(f"Percentage of genomes lost due to failed mapping: {lost_genomes_percentage:.1f}%\n\n")

                            f.write("=== FAILED GENERA DETAILS ===\n")
                            f.write("# Format: genus_name,domain,genome_count,percentage_of_total_genomes\n")

                            for _, row in final_failed.iterrows():
                                genome_pct = (row['genus_counts'] / total_genomes) * 100
                                f.write(f"{row['genus']},{row['domain']},{row['genus_counts']},{genome_pct:.2f}%\n")

                        logging.info(f"📝 Enhanced failed mapping log saved to: {log_file}")
                        logging.info(f"📊 Data loss summary: {final_failed_count}/{total_genera} genera ({final_failure_rate:.1f}%), {failed_genomes:,}/{total_genomes:,} genomes ({lost_genomes_percentage:.1f}%)")

                    # Show final statistics
                    total_genera = len(counts)
                    final_failed_count = len(final_failed)
                    final_success_rate = ((total_genera - final_failed_count) / total_genera) * 100
                    logging.info(f"📊 Final success rate: {final_success_rate:.1f}% ({total_genera - final_failed_count}/{total_genera})")
                    logging.info(f"📊 GTDB: {len(genus_to_taxid)}, NCBI fallback: {fallback_success_count}, Final failed: {final_failed_count}")

                    # Show top final failed matches by genome count
                    if not final_failed.empty:
                        top_failed = final_failed.nlargest(5, 'genus_counts')
                        logging.info("🔍 Top final failed matches by genome count:")
                        for _, row in top_failed.iterrows():
                            logging.info(f"  {row['genus']} ({row['domain']}): {row['genus_counts']} genomes")
                ...
                for _, row in counts.iterrows():
                    if pd.notna(row['taxid']):
                        ...
                        # If this genus was found in GTDB, use GTDB lineage
                        if genus_name in genus_to_taxid:
                ...
                # Add lineage columns using taxid lookup with genus validation
                lineages = []
                lineage_ranks = []
                lineage_taxids = []
                ...
                for _, row in counts.iterrows():
                    if pd.notna(row['taxid']):
                        ...
                        if taxid in all_lineage_data:
                            ...
                            # Clean up lineage to only include genus level and above
                            cleaned_lineage, cleaned_ranks, cleaned_taxids = cleanup_lineage_to_genus_level(
                ...
                # Additional grouping by last taxid to merge GTDB-specific genus names
                logging.info("Grouping by last lineage taxid to merge related genera...")
                ...
                for (last_taxid, domain), group in counts.groupby(['last_taxid', 'domain']):
                    if pd.isna(last_taxid):
                        ...
                        total_genomes = group['genus_counts'].sum()

                        # Use the most common genus name from the lineage, or first one
                        representative_row = group.iloc[0]

                        # Extract genus name from lineage if available
                        lineage = representative_row['lineage']
                        if lineage and ';' in lineage:
                            lineage_parts = lineage.split(';')
                            # Genus should be the 6th part (superkingdom;phylum;class;order;family;genus)
                            if len(lineage_parts) >= 6:
                                genus_name = lineage_parts[5]  # Sixth part should be genus
                            else:
                                genus_name = representative_row['genus']
                        else:
                            genus_name = representative_row['genus']
                ...
                logging.info(f"✅ After grouping by last taxid: {len(counts)} unique genera")
            ...
            with timer("Saving summary", metrics_list) as metrics:
                ...
                metrics.rows_processed = len(counts)
                # logging.info(f"✅ Saved {len(counts):,} genus-domain combinations")

            # Print preview
            logging.info("\n📈 Top 10 genera by genome count:")
