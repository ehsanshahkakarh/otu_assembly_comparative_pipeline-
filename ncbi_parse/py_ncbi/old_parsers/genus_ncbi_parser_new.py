...
This script processes NCBI assembly data to extract genus-level information.
...
- Taxonomic mapping file (taxid_to_genus.csv)

Output:
- Total genome counts (ncbi_genus_counts.csv)
- Species subset counts (ncbi_genus_species_counts.csv)

Usage:
    python genus_ncbi_parser.py [--input-dir INPUT_DIR] [--output-dir OUTPUT_DIR]
"""
...
def get_taxids_from_names(genus_names: List[str]) -> Dict[str, str]:
    """Get taxids for genus names using taxonkit name2taxid."""
    if not genus_names:
        return {}

    print(f"Getting taxids for {len(genus_names)} genus names...")
    name_to_taxid = {}
    ...
    print(f"Successfully processed {len(name_to_taxid)} genus name->taxid mappings")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Parse NCBI assembly data for genus-level counts")
    ...
    parser.add_argument("--mapping-file", help="Path to taxid mapping file (default: ../taxonomic_mapping/taxid_to_genus.csv)")
...
def setup_paths(args: argparse.Namespace) -> Tuple[Path, Path, Path, Path]:
    ...
    mapping_file = Path(args.mapping_file) if args.mapping_file else script_dir.parent / "taxonomic_mapping" / "taxid_to_genus.csv"
...
def main() -> int:
    """Main function to run the genus parser."""
    args = parse_arguments()

    try:
        # Set up logging
        log_file = setup_logging("genus_ncbi_parser")
        logging.info("Starting genus NCBI parser")
        ...
        total_counts = merged_df.groupby(["genus", "domain"]).size().reset_index(name="genus_count")
        total_counts = total_counts.sort_values("genus_count", ascending=False)
        ...
        # Group by genus and domain with proper counting logic (matching old script)
        print("Grouping by genus and domain...")
        if species_column == 'species_taxid':
            # For species_taxid, count unique taxids per genus
            species_counts = species_subset_df.groupby(["genus", "domain"]).agg({
        ...
        species_counts = species_counts.rename(columns={species_column: "genus_species_count"})
        species_counts = species_counts.sort_values("genus_species_count", ascending=False)

        # Get unique genus names for taxonkit processing
        unique_genera = total_counts['genus'].unique().tolist()
        print(f"\n🔍 Found {len(unique_genera)} unique genera")

        # Get taxids and lineages
        print("Getting taxids for genus names...")
        with tqdm(desc="Processing genus names", total=len(unique_genera), unit="genera") as pbar:
            genus_to_taxid = get_taxids_from_names(unique_genera)
            pbar.update(len(unique_genera))

        # Add taxid column to both count files
        total_counts['taxid'] = total_counts['genus'].map(genus_to_taxid)
        species_counts['taxid'] = species_counts['genus'].map(genus_to_taxid)

        # Get lineage information
        valid_taxids = [taxid for taxid in genus_to_taxid.values() if taxid]
        ...
        # Save count files
        total_output_file = output_dir / "ncbi_genus_counts.csv"
        species_output_file = output_dir / "ncbi_genus_species_counts.csv"

        print(f"💾 Saving total counts to {total_output_file}...")
        total_counts.to_csv(total_output_file, index=False)
        print(f"✅ Saved {len(total_counts)} genus-domain combinations (total counts)")

        print(f"💾 Saving species counts to {species_output_file}...")
        species_counts.to_csv(species_output_file, index=False)
        print(f"✅ Saved {len(species_counts)} genus-domain combinations (species counts)")

        # Print preview of results
        print(f"\n📈 Top 10 genera by total genome count:")
        for _, row in total_counts.head(10).iterrows():
            print(f"  {row['genus']} ({row['domain']}): {row['genus_count']} genomes")

        print(f"\n🧬 Top 10 genera by species count:")
        for _, row in species_counts.head(10).iterrows():
            print(f"  {row['genus']} ({row['domain']}): {row['genus_species_count']} species")

        logging.info("Genus NCBI parser completed successfully")
        return 0
