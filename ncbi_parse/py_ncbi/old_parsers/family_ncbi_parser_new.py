...
Output:
- Total genome counts (ncbi_family_counts.csv)
- Species subset counts (ncbi_family_species_counts.csv)
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Parse NCBI assembly data for family-level counts")
...
def main() -> int:
    """Main function to run the family parser."""
    ...
    try:
        ...
        total_counts = merged_df.groupby(["family", "domain"]).size().reset_index(name="family_count")
        total_counts = total_counts.sort_values("family_count", ascending=False)
        ...
        # Group by family and domain with proper counting logic (matching old script)
        print("Grouping by family and domain...")
        if species_column == 'species_taxid':
            # For species_taxid, count unique taxids per family
            species_counts = species_subset_df.groupby(["family", "domain"]).agg({
        ...
        species_counts = species_counts.rename(columns={species_column: "family_species_count"})
        species_counts = species_counts.sort_values("family_species_count", ascending=False)

        # Get unique family names for taxonkit processing
        unique_families = total_counts['family'].unique().tolist()
        ...
        total_counts['taxid'] = total_counts['family'].map(family_to_taxid)
        species_counts['taxid'] = species_counts['family'].map(family_to_taxid)
        ...
        # Save count files
        total_output_file = output_dir / "ncbi_family_counts.csv"
        species_output_file = output_dir / "ncbi_family_species_counts.csv"

        print(f"💾 Saving total counts to {total_output_file}...")
        total_counts.to_csv(total_output_file, index=False)
        print(f"✅ Saved {len(total_counts)} family-domain combinations (total counts)")

        print(f"💾 Saving species counts to {species_output_file}...")
        species_counts.to_csv(species_output_file, index=False)
        print(f"✅ Saved {len(species_counts)} family-domain combinations (species counts)")

        # Print preview of results
        print(f"\n📈 Top 10 families by total genome count:")
        for _, row in total_counts.head(10).iterrows():
            print(f"  {row['family']} ({row['domain']}): {row['family_count']} genomes")
