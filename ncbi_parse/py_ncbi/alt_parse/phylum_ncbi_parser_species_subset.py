#!/usr/bin/env python3
"""
NCBI Phylum Parser (Species Subset Version)

This script processes NCBI assembly data to extract phylum-level information.
It uses taxonomic mapping and groups by phylum and domain, similar to the eukprot parser format.
...
Output:
- Summary CSV with phylum species counts (ncbi_phylum_species_counts.csv)

Usage:
    python phylum_ncbi_parser_species_subset.py [--input-dir INPUT_DIR] [--output-dir OUTPUT_DIR]
"""
...
def get_taxids_from_names(phylum_names: List[str]) -> Dict[str, str]:
    ...
    Returns:
        Dictionary mapping phylum names to their taxids
    """
    if not phylum_names:
        return {}
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Parse NCBI assembly data for phylum-level information")
...
def setup_paths(args: argparse.Namespace) -> Tuple[Path, Path, Path]:
    ...
    output_file = output_dir / "ncbi_phylum_species_counts.csv"
    accession_output_file = output_dir / "ncbi_phylum_species_subset_with_accessions.csv"
...
def main() -> int:
    ...
    try:
        # Set up logging
        log_file = setup_logging("phylum_ncbi_parser_species_subset")
        logging.info("Starting phylum NCBI parser (species subset)")
        ...
        print(f"📂 Accession output file: {accession_output_file}")
        print(f"📂 Log file: {log_file}")

        logging.info(f"Input assembly file: {assembly_file}")
        logging.info(f"Input mapping file: {mapping_file}")
        logging.info(f"Output file: {output_file}")
        logging.info(f"Accession output file: {accession_output_file}")
        ...
        classified_file = output_dir_path / "ncbi_phylum_with_accessions_classified.csv"
        ...
        if classified_file.exists():
            ...
            # The assembly data uses 'assembly_accession' while classified data uses 'accession_clean'
        ...
        # Step 2: Group by phylum and domain, and count how many unique species
        print("Grouping by phylum and domain...")
        if species_column == 'species_taxid':
            # For species_taxid, count unique taxids per phylum
            counts = species_subset_df.groupby(["phylum", "domain"]).agg({
        ...
        counts = counts.rename(columns={species_column: "phylum_species_count"})
        ...
        print("Mapping taxids to phylum counts...")
        ...
        print(f"✅ Saved {len(counts)} phylum-domain combinations")
        ...
        accession_columns = ['assembly_accession', 'phylum', 'domain', 'taxid']
        ...
        logging.info("Phylum NCBI parser (species subset) completed successfully")
        return 0
