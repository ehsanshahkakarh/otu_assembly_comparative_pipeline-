...
Output:
- Summary CSV with phylum counts (ncbi_phylum_counts.csv)
...
def setup_paths(args: argparse.Namespace) -> Tuple[Path, Path, Path]:
    ...
    output_file = output_dir / "ncbi_phylum_counts.csv"
...
def main() -> int:
    ...
    try:
        ...
        # Step 1: Group by phylum and domain, and count total accessions (no species subsetting)
        print("Grouping by phylum and domain...")
        counts = merged_df.groupby(["phylum", "domain"]).size().reset_index(name="phylum_counts")
        ...
        detailed_output_file = output_file.parent / "ncbi_phylum_with_accessions.csv"
        ...
        columns_to_include = ['assembly_accession', 'phylum', 'domain', 'taxid']
