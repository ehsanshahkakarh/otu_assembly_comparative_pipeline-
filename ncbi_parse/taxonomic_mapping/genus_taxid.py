...
def load_taxonomy_files(taxdump_dir):
    ...
    try:
        ...
        with tqdm(desc="Loading nodes.dmp", unit="MB") as pbar:
            nodes_df = pd.read_csv(taxdump_dir / "nodes.dmp", sep="|", header=None)
            pbar.update(1)

        # Load names file with progress indication
        with tqdm(desc="Loading names.dmp", unit="MB") as pbar:
            names_df = pd.read_csv(taxdump_dir / "names.dmp", sep="|", header=None)
            pbar.update(1)

        # Clean up the data
        with tqdm(desc="Cleaning taxonomy data", unit="files") as pbar:
...
def main():
    ...
    # Set up logging
    setup_logging()
    logging.info("🚀 Starting optimized genus mapping process")

    # Load taxonomy files
    logging.info("Loading taxonomy files...")
    start_time = time.time()
    nodes_df, names_df = load_taxonomy_files(taxdump_dir)
    ...
    if use_parallel:
        ...
        result_df, unmapped_df = process_assembly_file_parallel(
            assembly_file, taxid_to_rank, taxid_to_parent, taxid_to_name, target_rank,
    ...
    candidatus_count = 0
    if len(result_df) > 0:
        candidatus_count = result_df[target_rank].str.contains("Candidatus", na=False).sum()

    # Save results - output file in script directory, unmapped in error_log
    output_file = script_dir / "taxid_to_genus.csv"
    unmapped_file = output_dir / f"unmapped_taxids_{target_rank}.csv"
