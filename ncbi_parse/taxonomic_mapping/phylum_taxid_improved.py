#!/usr/bin/env python3
"""
Improved phylum mapping script that addresses issues found in the original version.
This script uses taxonkit-like logic to ensure better lineage traversal.
"""
...
def parse_taxonkit_results(lineage_results):
    """Parse taxonkit lineage results and extract phylum information"""
        ...
        if len(parts) >= 3:
            ...
            if phylum_name:
                ...
                unmapped_results.append({
                    'taxid': taxid,
                    'reason': 'No phylum found in lineage',
                    'lineage': lineage,
                    'ranks': ranks
                })
        else:
    ...
    logging.info(f"✅ Found {len(mapped_results):,} entries with phylum information")
    logging.info(f"⚠️  Found {len(unmapped_results):,} entries without phylum information")
...
def save_results(mapped_results, unmapped_results, output_dir):
    ...
    if len(mapped_df) > 0:
        ...
        final_df = mapped_df[['taxid', 'phylum', 'domain']].copy()
    ...
    unmapped_file = error_dir / "unmapped_taxids_phylum.csv"
...
def compare_with_original(improved_file, original_file):
    """Compare results with original mapping"""
    try:
...
def main():
    setup_logging()
    logging.info("🚀 Starting improved phylum mapping process using taxonkit")
    ...
    # Compare with original backup if it exists
    original_backup_file = script_dir / "taxid_to_phylum_original_backup.csv"
