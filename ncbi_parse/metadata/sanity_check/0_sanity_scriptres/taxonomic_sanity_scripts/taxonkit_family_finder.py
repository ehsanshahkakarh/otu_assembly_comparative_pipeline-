#!/usr/bin/env python3
"""
Script to extract taxids from family unmapped entries, run them through taxonkit,
and identify entries that have family information in their taxonomic lineage.
"""
...
def parse_taxonkit_results(lineage_file):
    """Parse taxonkit lineage results and identify entries with families."""
    ...
    try:
        with open(lineage_file, 'r') as f:
            for line in tqdm(f, desc="Processing lineage results"):
                ...
                if len(parts) >= 3:
                    ...
                    # Check if 'family' or 'subfamily' is in the ranks
                    has_family = 'family' in ranks.lower() or 'subfamily' in ranks.lower()

                    # Extract family name if present
                    family_name = ""
                    if has_family and lineage and ranks:
                        lineage_parts = lineage.split(';')
                        rank_parts = ranks.split(';')

                        # First try to find 'family'
        ...
        print(f"🎯 Found {family_found_count} entries with family information")
    ...
    if 'species_taxid' in unmapped_df.columns:
        ...
        for idx in species_merge.index:
            if not pd.isna(species_merge.loc[idx, 'lineage_species']):
                ...
                merged_df.loc[idx, 'has_family'] = species_merge.loc[idx, 'has_family_species']
                merged_df.loc[idx, 'family_name'] = species_merge.loc[idx, 'family_name_species']
    
    print(f"✅ Merged data for {len(merged_df)} entries")
    
    return merged_df

def save_family_found_entries(merged_df, output_dir):
    """Save entries that have family information."""
    # Filter entries with family information
    family_entries = merged_df[merged_df['has_family'] == True].copy()
    
    if len(family_entries) == 0:
        print("⚠️  No entries with family information found")
        return
    
    # Sort by family name and organism name
    family_entries = family_entries.sort_values(['family_name', 'organism_name'])
    
    output_file = os.path.join(output_dir, "family_found_entries.csv")
    
    # Select relevant columns for output
    output_columns = [
        'assembly_accession', 'taxid', 'species_taxid', 'organism_name', 
        'infraspecific_name', 'isolate', 'assembly_level', 'genome_rep', 
        'asm_name', 'family_name', 'lineage', 'ranks'
    ]
    
    # Only include columns that exist
    available_columns = [col for col in output_columns if col in family_entries.columns]
    output_df = family_entries[available_columns]
    
    # Save with header statistics
    with open(output_file, 'w') as f:
        f.write("# UNMAPPED ENTRIES WITH FAMILY INFORMATION FOUND VIA TAXONKIT\n")
        f.write(f"# Total entries with family found: {len(family_entries)}\n")
        f.write(f"# Unique families found: {family_entries['family_name'].nunique()}\n")
        f.write("#\n")
        f.write("# TOP 10 FAMILIES BY ENTRY COUNT:\n")
        
        family_counts = family_entries['family_name'].value_counts().head(10)
        for i, (family, count) in enumerate(family_counts.items(), 1):
            f.write(f"# {i:2d}. {family:<35} {count:>6} entries\n")
    ...
    print(f"💾 Saved {len(family_entries)} entries with family info to: {output_file}")
    
    # Show summary
    print(f"\n📊 FAMILY SUMMARY:")
    print(f"   Total entries with family: {len(family_entries)}")
    print(f"   Unique families found: {family_entries['family_name'].nunique()}")
    print(f"\n🏆 Top 5 families:")
    for i, (family, count) in enumerate(family_counts.head().items(), 1):
        print(f"   {i}. {family}: {count} entries")
...
def main():
    ...
    unmapped_file = script_dir.parent / "family_sanity" / "family_unmapped_entries.csv"
    output_dir = script_dir.parent / "family_sanity"
    
    print("🚀 Starting taxonkit family finder...")
    ...
    if not os.path.exists(unmapped_file):
        ...
        print("Please run family_sanity_check.py first to generate unmapped entries.")
        sys.exit(1)
    ...
    # Save entries with family information
    save_family_found_entries(merged_df, output_dir)
    
    # Cleanup temporary files
    cleanup_temp_files(output_dir)
    
    print("\n✅ Taxonkit family finder completed successfully!")
