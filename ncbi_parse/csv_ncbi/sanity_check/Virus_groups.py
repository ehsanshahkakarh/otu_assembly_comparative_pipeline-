...
def process_viral_groups():
    """Process CSV files to extract unique taxonomic groups for viruses with their taxids."""
    ...
    output_files = {
        'genus': output_dir / "viral_genera.csv",
        'family': output_dir / "viral_families.csv",
        'phylum': output_dir / "viral_phyla.csv"
    }

    print("📥 Processing viral taxonomic groups...")
    ...
    for level, input_file in input_files.items():
        try:
            ...
            # Filter for Viruses (case insensitive, no leading/trailing spaces)
            filtered_df = df[df['domain'].str.strip().str.lower().isin(['viruses', 'virus'])]
            ...
            print(f"✅ Saved {len(result_df)} unique Viral {level} with taxids to: {output_files[level].name}")
