#!/usr/bin/env python3
"""
Parse EukCensus 16S Clusters TSV File

This script parses the eukcensus_16S.clusters.97.tsv file (metadata) and generates three CSV files
organized by phylum, family, and genus. Each file contains columns for taxid, taxon_name,
member_size, and an array of centroid entries.
...
def should_filter_taxon(taxon_name):
    ...
    if any(pattern in taxon_name for pattern in [".U.phylum", ".U.genus", ".U.family", ".U.order", ".U.class", ".U.species"]):
        return True
...
def main():
    ...
    # Output file names
    phylum_output = os.path.join(output_dir, "eukcensus_by_phylum.csv")
    ...
    # Note: There's a typo in the column name 'familiy' instead of 'family'
    # We'll use the column name as it appears in the file

    # Initialize dictionaries to store grouped data
    phylum_data = defaultdict(lambda: {'size': 0})
    family_data = defaultdict(lambda: {'size': 0})
    genus_data = defaultdict(lambda: {'size': 0})
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Grouping by taxonomic ranks"):
        ...
        family = row['familiy'] if not pd.isna(row['familiy']) else "Unknown"  # Note the typo in the column name
        genus = row['genus'] if not pd.isna(row['genus']) else "Unknown"

        # Update phylum data
        phylum_data[phylum]['size'] += size
    ...
    # Get taxids for phyla
    print("Getting taxids for phyla...")
    phylum_names = list(phylum_data.keys())
    phylum_to_taxid = get_taxids_for_names(phylum_names)
    ...
    # Process phylum taxids
    phylum_entries = []
    filtered_phylum_count = 0
    for phylum, data in phylum_data.items():
        # Check if this taxon should be filtered out
        if should_filter_taxon(phylum):
            filtered_phylum_count += 1
            continue
        ...
        if "." in phylum:
            organism_part = phylum.split(".")[0]
            if organism_part in phylum_to_taxid:
                organism_taxid = phylum_to_taxid[organism_part]
        ...
        phylum_entries.append((phylum, data, taxid))
    ...
    # Write phylum data to CSV
    print(f"Writing phylum data to {phylum_output}")
    with open(phylum_output, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['taxid', 'taxon_name', 'domain', 'member_size'])

        for phylum, data, taxid in phylum_entries:
            # Since this is 16S data, default to Bacteria
            domain = "Bacteria"
                ...
                if organism_part in phylum_to_taxid:
                    ...
                    if organism_taxid in domain_cache:
                        ...
                        if organism_domain == "Eukaryota":
                            print(f"✅ Keeping organelle entry in phylum data: {phylum} (organism part: {organism_part} is eukaryotic)")
            ...
            writer.writerow([taxid, phylum, domain, data['size']])

        if filtered_phylum_count > 0:
            print(f"Filtered out {filtered_phylum_count} phyla with unidentified taxon patterns")
    ...
    with open(family_output, 'w', newline='') as f:
        ...
        for family, data, taxid in family_entries:
            ...
            writer.writerow([taxid, family, domain, data['size']])
    ...
    with open(genus_output, 'w', newline='') as f:
        ...
        for genus, data, taxid in genus_entries:
            ...
            writer.writerow([taxid, genus, domain, data['size']])
