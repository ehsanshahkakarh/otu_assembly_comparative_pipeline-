...
if 'phylum' not in df.columns:
    raise ValueError("Column 'phylum' not found in the input file.")


# Extract and save both taxon_name and data_source to the same file
df[['phylum']].drop_duplicates().to_csv(args.output_file, index=False, header=True)
print(f"✅ Extracted phylum columns saved to: {args.output_file}")
